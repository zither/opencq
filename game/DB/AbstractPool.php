...
abstract class AbstractPool
{
    ...
    public function get($timeOut = 3)
    {
        ...
        while (1) {
            $obj = $this->connections->pop($this->toMicroseconds($timeOut));
            if (!$obj) {
                error_log(sprintf('connection pop failed in method get:%s', static::class));
                return false;
            }
        ...
        // 计算当前使用峰值
        $_max_used_con_num = $this->count - $this->connections->getLength();
    ...
    {
        // 大约1分钟检测一次连接
        $spareTime = $this->spareTime;
        // 回收算法，获得一分钟内被使用的连接峰值。当前count-峰值，如果大于等于2。那么释放一个连接
        Coroutine::run(function () use ($spareTime) {
            try {
                while (true) {
                    //error_log(sprintf('%s current count:%d', static::class, $this->connections->getLength()));
                    // 如果满足这个条件，那么释放一个连接
                    if ($this->count > $this->min && ($this->count - $this->max_used_con_num) >= 2) {
                        // 弹出一个资源，如果成功，那么去掉
                        $obj = $this->connections->pop($this->toMicroseconds(0.001));
                        if ($obj) {
                            $this->closeDb($obj);
                            $this->count--;
                            error_log(sprintf('decr count in gc: %d, max:%d', $this->count, $this->max));
                        } else {
                            error_log('pop error in gc');
                        }
                    }
    ...
    protected function toMicroseconds(float $seconds): float
    {
        return $seconds * 1000;
    }
}