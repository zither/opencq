...
<div class="flex flex-col" id="main-container">
    ...
    <div>
        <div class="my-4">
            <a class="px-2 py-1 rounded border border-gray-400 text-blue-500" href="<?=$this->_l('cmd=select-combat-skill&from=3&cid=%d', $combatStatus->id)?>">选择技能</a>
            <?php foreach($skills as $v) : ?>
                ...
                <?php elseif ($v['type'] == 1): ?>
                    <a class="px-2 py-1 rounded border border-gray-400 text-blue-500" href="<?=$this->_l('cmd=select-combat-skill&from=3&cid=%d', $combatStatus->id)?>"><?=$v['name']?></a>
                <?php else: ?>
                    <a class="px-2 py-1 rounded border border-gray-400 text-blue-500" href="<?=$this->_l('cmd=do-defense&id=%d&action=skill&aid=%d&nowmid=%d', $combatStatus->id, $v['id'], $player->nowmid)?>"><?=$v['name']?></a>
                <?php endif; ?>
            <?php endforeach; ?>
        </div>
        <div class="my-4">
            <a class="px-2 py-1 rounded border border-gray-400 text-blue-500" href="<?=$this->_l('cmd=select-combat-item&from=3&cid=%d', $combatStatus->id)?>">选择物品</a>
            <?php foreach($medicines as $v) : ?>
                ...
                <?php elseif ($v['type'] == 1): ?>
                    <a class="px-2 py-1 rounded border border-gray-400 text-blue-500" href="<?=$this->_l('cmd=select-combat-item&from=3&cid=%d', $combatStatus->id)?>"><?=$v['name']?></a>
                <?php else: ?>
                    <a class="px-2 py-1 rounded border border-gray-400 text-blue-500" href="<?=$this->_l('cmd=do-defense&id=%d&action=item&aid=%d&nowmid=%d', $combatStatus->id, $v['id'], $player->nowmid)?>"><?=$v['name']?></a>
                <?php endif; ?>
            <?php endforeach; ?>
        </div>
    </div>
